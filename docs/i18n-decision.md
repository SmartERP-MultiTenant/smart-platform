...
- **Namespaces:** `common` (app shell + registration funnel + ERP subscription + shared chrome such as `switch-theme`, which is used by the app-shell Header) and `marketing` (landing page copy — all `landing-*` keys). Landing components use `useTranslation('marketing')`; `LandingHeader` requests `useTranslation(['marketing', 'common'])`; the rest of the app uses `common`.
