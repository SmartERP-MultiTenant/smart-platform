...
- **Namespaces:** `common` (app shell + registration funnel + ERP subscription + shared chrome such as `switch-theme`, which is used by both the app-shell Header and the landing FenoiseHeader) and `marketing` (landing page copy — all `landing-*` keys). Landing components use `useTranslation('marketing')`; FenoiseHeader requests `useTranslation(['marketing', 'common'])`; the rest of the app uses `common`.
