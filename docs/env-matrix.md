...
| `RATE_LIMIT_TRUSTED_HOPS`           | 1 (default) | 2 (verify against nginx)                                    | 2 (verify against nginx)                                                       | runtime   | [DEF]         | optional               | Trusted reverse-proxy hops for the public-funnel bucket key (P4.22). Client IP is read from the RIGHT of `X-Forwarded-For`; `0` disables it. Verify the real hop count against `/etc/nginx/` before go-live      |
