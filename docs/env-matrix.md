...
| `ERP_TOKEN_ENCRYPTION_KEY` | not set (dev-only fallback + one-time warn, `lib/crypto/erpToken.ts`)           | [GHS:staging ENV_ERP_TOKEN_ENCRYPTION_KEY] (generate: `openssl rand -base64 32`) | [GHS:production ENV_ERP_TOKEN_ENCRYPTION_KEY] | runtime   | [LOC] + [GHS] | placeholder | **Data-at-rest key** for `Team.erpAccessToken` (AES-256-GCM). Required-at-first-use, not boot: encrypt/decrypt throw in production when unset (never a published fallback). `NEXTAUTH_SECRET` is NOT used (key separation). Envelope `enc:v1:<keyId>:…` is rotation-ready (P4.14 fix) |
...
| `RATE_LIMIT_TRUSTED_HOPS`           | 2 (default) | 2 (confirmed 2026-09-14)                                    | 2 (confirmed 2026-09-14)                                                       | runtime   | [DEF]         | real                   | Trusted reverse-proxy hops for the public-funnel bucket key (P4.22). Client IP is read from the RIGHT of `X-Forwarded-For`; `0` disables it. Confirmed topology **Cloudflare → nginx → app** (owner sign-off 2026-09-14), which is why the code default matches production rather than the smallest possible chain: a value too LOW keys every client behind that proxy into ONE bucket (a global 10/min ceiling on `register`/`payments`) |
| `RECAPTCHA_SITE_KEY`                | empty (off) | [GHS:staging] pair optional (real test keys only)           | [GHS:production] pair optional — **real keys only, both-or-neither**           | runtime   | [LOC] + [GHS] | conditional(both keys) | Public site key; **placeholder values would ENABLE captcha** with invalid keys and break credentials login (`lib/recaptcha.ts:5` — activates when both non-empty). Absent pair → CI renders explicit empty (off)                                                                                                                                                                                                                           |
| `RECAPTCHA_SECRET_KEY`              | empty (off) | [GHS:staging ENV_RECAPTCHA_SECRET_KEY] (real test key only) | [GHS:production ENV_RECAPTCHA_SECRET_KEY] (real key only — both-or-neither)    | runtime   | [LOC] + [GHS] | conditional(both keys) | Secret — never in git; pair rule as site key (see row above)                                                                                                                                                                                                                                                                                                                                                                               |
...
| `CRON_SECRET`           | not set (route returns 503) | [GHS:staging ENV_CRON_SECRET] | [GHS:production ENV_CRON_SECRET] | runtime   | [LOC] + [GHS]                             | placeholder | Scheduled-route guard (`/api/cron/*`, e.g. `renewal-reminders`). **No open mode**: endpoint disabled (503) when unset. Header auth only (`Authorization: Bearer` / `x-cron-secret`, constant-time compare) — never query string (log-leak). Provision `ENV_CRON_SECRET` per environment (D2-C) |
...
| `NEXT_PUBLIC_SUPPORT_URL`    | WhatsApp `wa.me/966507501490` | same (via image build-arg) | same (via image build-arg)                             | **build** | `container` job `build-args` (`.github/workflows/main.yml`) | real        | Owner-approved 2026-09-14 (P1.3 `86cbbpyey`), WhatsApp Business. Baked into the image at build time (D6) by the `container` job's build-arg — a **runtime** `.env` value or compose `environment:` entry is inert, so changing the number needs a rebuild + redeploy. Empty → every contact CTA is omitted rather than rendered as a dead link |
| `NEXT_PUBLIC_TWITTER_HANDLE` | empty                         | empty                      | real brand handle once the owner supplies it           | **build** | [DEF] fallback `''`                                         | placeholder | Normalised to a single leading `@` (SEO.tsx:37); unset/blank/`@`-only → `twitter:site` **omitted**, never emitted empty (SEO.tsx:111). Raw `process.env` read (SEO.tsx:50), **not** `lib/env.ts`                                                                                                                                               |
| `NEXT_PUBLIC_APP_STORE_URL`  | empty                         | empty                      | real App Store URL when the iOS app is published       | **build** | [DEF] unset → non-interactive                               | placeholder | Unset → the badge renders non-interactive: "coming soon" `span` + `aria-disabled`, never an anchor without `href` (MobileSection.tsx:83,98; raw `process.env` read at :10)                                                                                                                                                                     |
| `NEXT_PUBLIC_PLAY_STORE_URL` | empty                         | empty                      | real Google Play URL when the Android app is published | **build** | [DEF] unset → non-interactive                               | placeholder | Same rendering contract as the App Store row above (MobileSection.tsx:11; render gate :83,:98)                                                                                                                                                                                                                                                 |
...
| `JACKSON_API_KEY`        | [LOC]                          | [GHS:staging ENV_JACKSON_API_KEY] placeholder        | [GHS:production ENV_JACKSON_API_KEY]                                                                          | runtime   | [LOC] + [GHS]               | conditional(FEATURE_TEAM_SSO)      | Secret (lib/env.ts:73; scripts/delete-team.js:36). CI supplies test values |
...
| `STRIPE_SECRET_KEY`      | empty (payments off)           | empty                                                | empty — payments stack decision is Moyasar/Tabby/Tamara; Stripe vars stay unused                              | runtime   | [LOC] + [GHS]               | conditional(FEATURE_TEAM_PAYMENTS) | Secret (lib/env.ts:120,135; scripts/sync-stripe.js)                        |
