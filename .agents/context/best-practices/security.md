...
- **Auth hardening exists:** `lib/accountLock.ts` — login-attempt throttling (`MAX_LOGIN_ATTEMPTS`, default 5), account lockout + unlock email (7-day token), `lib/server-common.ts` `generateToken(64)` uses `crypto.randomBytes` (not Math.random). Email confirmation + disable-non-business-email-signup flags in env.
- **API surface discipline:** every API route handles unknown methods with `405` + `Allow` header, wraps handlers in try/catch → `ApiError`/500, uses `getCurrentUser`/`requireTeamMembership` guards (`lib/guards/`), and forces response consumption (`forceConsume`) to avoid leaks. Team-scoped routes must verify membership via the `Team`/`TeamMember` model — never trust `teamSlug` alone.
