...
- **Secrets live only in `.env`** (gitignored). `NEXTAUTH_SECRET`, Stripe, Jackson, Svix, SMTP, reCAPTCHA keys — accessed via `lib/env.ts` only, never hardcoded, never shipped (exception: the standalone scripts `scripts/sync-stripe.js` and `scripts/delete-team.js` read env directly via `process.env`). `NEXT_PUBLIC_*` vars are the ONLY client-visible ones (mixpanel token, dark mode).
