...
| `NEXT_PUBLIC_DARK_MODE`                                    | false — keep an explicit value everywhere: read `!== 'false'` (`lib/env.ts:108`), so **unset = dark mode ON**; build-time, runtime `.env` cannot change it. The **public landing** no longer renders a theme toggle at all (P4.18) — the flag now only gates the authenticated account/app shell (`pages/_app.tsx:44`, `components/shared/shell/Header.tsx:166`, `components/account/UpdateAccount.tsx:19`) |
