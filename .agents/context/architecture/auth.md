<!-- Context: architecture/auth | Priority: high | Version: 1.2 | Updated: 2026-09-13 -->
...
- Admin pages (`pages/admin.tsx`, `pages/admin/revenue.tsx`, `pages/admin/users.tsx`, `pages/admin/tenants/[teamId].tsx`): SSR guard — ApiError 401 → login redirect, 403 → forbidden state, anything else rethrown (500). Arabic RTL (`dir="rtl" lang="ar"`, funnel convention) that reuses **existing** `admin-*` locale keys and adds none (locked decision 2026-09-02; P1.2 owns i18n).
...
- Guard-first, GET-only API: `pages/api/admin/dashboard.ts` and `pages/api/admin/tenants/[teamId].ts` call `requirePlatformAdmin` **before** any method or data logic (405 on other methods, 422 invalid id, 404 unknown team). Mutations, the `AdminAuditLog` store and revenue aggregation are P5.4+ and deliberately absent here.
