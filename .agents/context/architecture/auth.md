...
- Account security: `MAX_LOGIN_ATTEMPTS` (lockout via `lib/accountLock.ts`), `User.disabledAt` (admin-level account disable rejected at login via `lib/nextAuth.ts`), reCAPTCHA hooks (`lib/recaptcha.ts`), email verification (`CONFIRM_EMAIL`), non-business-email signup gate (`DISABLE_NON_BUSINESS_EMAIL_SIGNUP`).
...
- `User.disabledAt DateTime?`: Admin-level permanent account disable. Enforced in `lib/nextAuth.ts` `authorize` before password check.
- Platform Admin User Management API (`/api/admin/users`):
  ...
  - Audit logging seam in `lib/adminAudit.ts`.
...
- `lib/auth.ts` · `lib/nextAuth.ts` · `middleware.ts` · `pages/auth/*` · `lib/jackson*` · `lib/guardPlatformAdmin.ts` · `lib/adminAudit.ts` · `pages/api/admin/users/*` · `scripts/seed-platform-admin.ts` · `docs/platform-admin-bootstrap.md`
